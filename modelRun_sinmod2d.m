    ...
    methods
        ...
        function run = modelRun_sinmod2d(thepath,filenameTemplate,depthRange,dtOut)
            ...
            tab = {...
                ...
                'flagel', 'PhytoF'; ...          % small phytoplankton
                'diatom', 'PhytoD'; ...          % large phytoplankton
                'PS'   ,  'PhytoF'; ...          % small phytoplankton; kept for easy compatibility with BIOMAS
                'PL'   ,  'PhytoD'; ...          % large phytoplankton; kept for easy compatibility with BIOMAS
                'NO3',    'NO3'};                % nitrate
...
%                 disp(run.filenames{i});
        ...
        function c = read(run,localVarname,n)
            [I,J] = size(run.grid.x);
            vi = strcmp(localVarname,run.vars.local);
            if sum(vi)==0
                c = [];
...
%                    ' at step ' num2str(ind) '-1']);
            ...
            try
                ...
                c = c .* double(scale) + double(offset);
            catch
                % attribute does not exist => do nothing
            end
            c(c>1e16) = nan;
            % apply NetCDF fill/missing values if available
            try
                val = netcdf.getAtt(nc,varid,'_FillValue');
                c(c==val) = nan;
            catch
                % attribute does not exist => do nothing
            end
            try
                val = netcdf.getAtt(nc,varid,'missing_value');
                c(c==val) = nan;
            catch
                % attribute does not exist => do nothing
            end
        ...
        function run = loadFrame(run,n,tracers)
            ...
            for i=1:length(fields)
                if isnumeric(run.F1.(fields{i}))
                    ...
                    run.F1.si.(fields{i}) = ...
                        scatteredInterpolant(run.grid.x(:), run.grid.y(:), C(:));
                end
            end    
        end
